 ...
 *
 * Usage:
 *   npx tsx backfill-descriptions.ts --batch 1000                          # first 1000 (global)
 *   npx tsx backfill-descriptions.ts --batch 1000 --all                    # until done (global)
 *   npx tsx backfill-descriptions.ts --ids-file=/tmp/backfill-target-ids.json --batch 1000        # targeted IDs, first 1000
 *   npx tsx backfill-descriptions.ts --ids-file=/tmp/backfill-target-ids.json --batch 1000 --all  # targeted IDs, until done
 *   npx tsx backfill-descriptions.ts --status                              # show global progress
...
const IDS_FILE = process.argv.find((a) => a.startsWith("--ids-file="))?.split("=")[1] ?? null;
...
async function getDbHealth(client: pg.Client, ids: number[] | null): Promise<DbHealth> {
  ...
  const { rows } = await client.query<DbHealth>(`
    SELECT
      ...
      (SELECT count(*) FROM products_v2 WHERE description IS NULL AND removed_at IS NULL ${filter})::int AS remaining_null,
      (SELECT count(*) FROM products_v2 WHERE description IS NOT NULL AND removed_at IS NULL ${filter})::int AS migrated,
      (SELECT count(*) FROM products_v2 WHERE description IS NOT NULL AND description != '' AND removed_at IS NULL ${filter})::int AS migrated_with_text
  `);
  return rows[0];
}
...
async function main() {
  ...
  const before = await getDbHealth(monitorClient, targetIds);
  ...
  do {
    ...
    const after = await getDbHealth(monitorClient, targetIds);
  ...
  // Final health
  const final = await getDbHealth(monitorClient, targetIds);
