    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="w-full max-w-2xl rounded-2xl bg-white shadow-2xl max-h-[85vh] flex flex-col">
        ...
        <div className="flex items-center justify-between border-b border-kv-gray-200 px-6 py-4">
          <div>
            <h3 className="text-base font-semibold text-kv-gray-900">Detail položky</h3>
          ...
          <button
            onClick={onClose}
            className="flex h-8 w-8 items-center justify-center rounded-lg text-kv-gray-400 transition-colors hover:bg-kv-gray-100 hover:text-kv-gray-600"
          >
        ...
        <div className="border-b border-kv-gray-100 bg-kv-gray-50 px-6 py-3">
          <div className="flex items-center justify-between">
            <div>
              <span className="text-xs text-kv-gray-400">Z poptávky:</span>
              <p className="text-sm font-medium text-kv-gray-800">{item.originalName}</p>
            </div>
            <div className="flex items-center gap-3">
              {item.quantity && (
                <span className="text-sm text-kv-gray-500">{item.quantity} ks</span>
              )}
        ...
        <div className="border-b border-kv-gray-100 px-6 py-3">
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-lg border border-kv-gray-200 bg-white py-2 pl-10 pr-10 text-sm outline-none transition-colors placeholder:text-kv-gray-400 focus:border-kv-red/30 focus:ring-2 focus:ring-kv-red/10"
            />
            {searchQuery && (
              <button
                onClick={handleClearSearch}
                className="absolute right-2 top-1/2 -translate-y-1/2 flex h-6 w-6 items-center justify-center rounded-md text-kv-gray-400 hover:bg-kv-gray-100 hover:text-kv-gray-600"
              >
          ...
          {searchQuery.length > 0 && searchQuery.length < MIN_SEARCH_LENGTH && (
            <p className="mt-1 text-[11px] text-kv-gray-400">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar px-6 py-3">
          <p className="mb-2 text-xs font-medium text-kv-gray-400 uppercase tracking-wide">
              ...
              <button
                ...
                className={`w-full rounded-xl border px-4 py-3 text-left transition-all ${
                  selectedProduct?.sku === product.sku
                    ? "border-kv-red bg-kv-red-light/50 ring-1 ring-kv-red/20"
                ...
                <div className="flex items-center justify-between">
                  <div className="min-w-0">
                    <div className="flex items-center gap-1.5">
                      <p className="text-sm font-medium text-kv-gray-800 truncate">{product.name}</p>
                  ...
                  {selectedProduct?.sku === product.sku && (
                    <div className="ml-3 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-kv-red text-white">
          ...
          <div className="mt-4 border-t border-kv-gray-100 pt-4">
            ...
            <input
              ...
              className="w-full rounded-lg border border-kv-gray-200 bg-white px-3 py-2 font-mono text-sm outline-none transition-colors placeholder:text-kv-gray-400 focus:border-kv-red/30 focus:ring-2 focus:ring-kv-red/10"
            />
          </div>
        </div>
        ...
        <div className="flex items-center justify-between border-t border-kv-gray-200 px-6 py-4">
          <button
            onClick={() => onSkip(item)}
            className="rounded-lg border border-kv-gray-200 px-4 py-2 text-xs font-medium text-kv-gray-500 transition-colors hover:bg-kv-gray-50"
          >
            Přeskočit
          </button>
          <div className="flex gap-2">
            <button
              onClick={onClose}
              className="rounded-lg border border-kv-gray-200 px-4 py-2 text-xs font-medium text-kv-gray-600 transition-colors hover:bg-kv-gray-50"
            >
              Zrušit
            </button>
            <button
              ...
              className="rounded-lg bg-kv-red px-5 py-2 text-xs font-medium text-white transition-colors hover:bg-kv-red-dark disabled:opacity-40 disabled:cursor-not-allowed"
            >
              Potvrdit výběr
            </button>
          </div>
